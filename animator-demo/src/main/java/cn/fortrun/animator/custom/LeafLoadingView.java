...
public class LeafLoadingView extends View {
    ...
    private void drawLeafs(Canvas canvas) {
        ...
        for (int i = 0; i < mLeafInfos.size(); i++) {
            ...
            if (now > leaf.startTime && leaf.startTime != 0) {
                // 绘制叶子－－根据叶子的类型和当前时间得出叶子的（x，y）
                getLeafLocation(leaf, now);
                // 根据时间计算旋转角度
                canvas.save();
                // 通过Matrix控制叶子旋转
                ...
                float rotateFraction = ((now - leaf.startTime) % mLeafRotateTime)
    ...
    private void getLeafLocation(Leaf leaf, long time) {
        long intervalTime = time - leaf.startTime;
